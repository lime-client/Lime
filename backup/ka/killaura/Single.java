...
import lime.features.module.impl.combat.AutoPot;
...
public class Single extends KillAuraMode {
    ...
    public void onMotion(EventMotion e) {
        if(KillAura.isBlocking && killAura.hasSword() && killAura.autoBlock.is("none")) {
            mc.thePlayer.sendQueue.addToSendQueue(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));
            mc.playerController.syncCurrentPlayItem();
            KillAura.isBlocking = false;
        }
        ...
        if(entity != null && killAura.isValid(entity)) {
            ...
            if(!killAura.rotations.is("none")) {
                ...
                switch(killAura.rotations.getSelected().toLowerCase()) {
                    ...
                    case "smooth":
                        ...
                        Rotation rotation = CombatUtils.smoothAngle(rotations, crt, (float) killAura.rotationsSpeedMin.getCurrent(), (float) killAura.rotationsSpeedMax.getCurrent());
                        if(killAura.gcd.isEnabled()) {
                ...
                if(killAura.autoBlockState.is(e.getState().name()) && killAura.hasSword() && (killAura.autoBlock.is("basic") || killAura.autoBlock.is("still")) && !KillAura.isBlocking) {
            ...
            if(cpsTimer.hasReached(20 / cps * 50L) && mc.thePlayer.getDistanceToEntity(entity) <= killAura.range.getCurrent() && killAura.state.is(e.getState().name())) {
                EventAttack ea = new EventAttack(entity);
                EventBus.INSTANCE.call(ea);
