...
public class NumberSetting extends Component {
    public NumberSetting(int x, int y, FrameModule owner, Setting setting) {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        NumberProperty slide = (NumberProperty) getSetting();
        ...
        FontManager.ProductSans20.getFont().drawString(getSetting().getSettingName() + ": " + roundToPlace(((NumberProperty) getSetting()).getCurrent(), 2), x + 5, y + (getOffset() / 2F - (FontManager.ProductSans20.getFont().getFontHeight() / 2F)), stringColor, true);
    }
    ...
    private void setValue(double value) {
        final NumberProperty set = (NumberProperty) getSetting();
