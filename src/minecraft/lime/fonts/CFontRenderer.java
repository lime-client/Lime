...
public class CFontRenderer extends FontRenderer {
    ...
    public void drawCenteredString(String s, int x, int y, int color) {
        if (Minecraft.getMinecraft().getLanguageManager().isCurrentLocaleUnicode() || false) {
            Minecraft.getMinecraft().fontRendererObj.drawString(s, x, y, color);
        }
    ...
    public int func_175063_a(String message, float posX, float posY, int textColor) {
        return Minecraft.getMinecraft().getLanguageManager().isCurrentLocaleUnicode() ? Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(message, (float) ((int) posX), (float) ((int) posY), textColor) : this.drawStringWithShadow(message, posX, posY, textColor);
    }
                ...
                for (int index = 0; index < parts.length; ++index) {
                    if (parts[index].length() > 0) {
                        if (index == 0) {
                            ...
                            if (bold && italic) {
                                this.boldItalicFont.drawString(randomCase ? this.toRandom(currentFont, words) : words, width, 0.0D, currentColor, shadow);
                                currentFont = this.boldItalicFont;
                            } else if (bold) {
                                this.boldFont.drawString(randomCase ? this.toRandom(currentFont, words) : words, width, 0.0D, currentColor, shadow);
                                currentFont = this.boldFont;
                            } else if (italic) {
                                this.italicFont.drawString(randomCase ? this.toRandom(currentFont, words) : words, width, 0.0D, currentColor, shadow);
                                currentFont = this.italicFont;
                            } else {
                                this.font.drawString(randomCase ? this.toRandom(currentFont, words) : words, width, 0.0D, currentColor, shadow);
                            ...
                            if (strikethrough) {
                                this.drawLine((double) width / 2.0D + 1.0D, h / 3, (double) (width + currentFont.getStringWidth(words)) / 2.0D + 1.0D, h / 3, u);
                            }

                            if (underline) {
                                this.drawLine((double) width / 2.0D + 1.0D, h / 2, (double) (width + currentFont.getStringWidth(words)) / 2.0D + 1.0D, h / 2, u);
                            }
    ...
    public int getStringHeight(String text) {
        if (text == null) {
            ...
            return !Minecraft.getMinecraft().getLanguageManager().isCurrentLocaleUnicode() && true ? this.font.getStringHeight(text) : Minecraft.getMinecraft().fontRendererObj.FONT_HEIGHT;
        }
    }

    public int getHeight() {
        return !Minecraft.getMinecraft().getLanguageManager().isCurrentLocaleUnicode() && true ? this.font.getHeight() : Minecraft.getMinecraft().fontRendererObj.FONT_HEIGHT;
    }
    ...
    public int getCharWidth(char c) {
        return !Minecraft.getMinecraft().getLanguageManager().isCurrentLocaleUnicode() && true ? this.getStringWidth(Character.toString(c)) : Minecraft.getMinecraft().fontRendererObj.getCharWidth(c);
    }
