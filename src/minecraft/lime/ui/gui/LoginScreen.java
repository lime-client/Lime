...
public class LoginScreen extends GuiScreen {
            ...
            try {
                new Thread(() -> {
                    try {
                        ...
                        final String response = WebUtils.getSource(s + cipherEncryption.encrypt(userName) + s1 + cipherEncryption.encrypt(MainScreen.getHardwareID()) + "&key=" + cipherEncryption.encrypt(key) + "&valid=" + cipherEncryption.encrypt("true"));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if(Lime.getInstance().getUser() != null && timer.hasReached(300)) {
