...
public class Scaffold extends Module {
    ...
    public Timer timerMotion = new Timer();
    public float yaw;
    public float pitch;
    ...
    private static List<Block> invalid = Arrays.asList(new Block[] { Blocks.air, Blocks.water, Blocks.flowing_water, Blocks.lava, Blocks.flowing_lava,
            ...
            Blocks.stone_button, Blocks.wooden_button, Blocks.lever, Blocks.tnt });
    ...
    public Scaffold() {
        ...
        mods.add("Basic");
        mods.add("NCP/Hypixel");
        mods.add("Cubecraft");

        ArrayList<String> f = new ArrayList<>();
        f.add("Spoof");
        f.add("LiteSpoof");
        Lime.setmgr.rSetting(new Setting("Scaffold Mode", this, "NCP/Hypixel", mods));
        Lime.setmgr.rSetting(new Setting("Spoof", this, "Spoof", f));
    }
    ...
    public void onEnable(){
        ...
        super.onEnable();
        oof = new ScaledResolution(this.mc);
    ...
    public int getBlockCount(){
        ...
        for(int i = 9; i < 45; i++){
            if(mc.thePlayer.inventoryContainer.getSlot(i).getHasStack()){
                ItemStack issoufre = mc.thePlayer.inventoryContainer.getSlot(i).getStack();
                if(issoufre.getItem() instanceof ItemBlock){
                    blockIssou = blockIssou + issoufre.stackSize;
                }
            }
        }
        return blockIssou;
    }
    ScaledResolution oof = new ScaledResolution(this.mc);
    @EventTarget
    public void onRender(Event2D e){

        Lime.fontManager.comfortaa_hud.drawString(getBlockCount() + " blocks", oof.getScaledWidth() / 2 - 20, oof.getScaledHeight() / 2 + Lime.fontManager.comfortaa_hud.FONT_HEIGHT, -1, false);
    ...
    public void onMotion(EventMotion e){
        try{
            ...
            boolean bool1 = true;
            boolean bool2 = true;
            if(getSettingByName("Scaffold Mode").getValString().equalsIgnoreCase("NCP/Hypixel")){
                e.setPitch(90);
                e.setYaw(360);
            }
            if(getSettingByName("Scaffold Mode").getValString().equalsIgnoreCase("Cubecraft")){
                if(mc.thePlayer.isSprinting()){
                    mc.thePlayer.setSprinting(false);
                    mc.timer.timerSpeed = 0.8F;
                    mc.thePlayer.rotationYawHead = 999F;
                }
            }
            if (mc.gameSettings.keyBindJump.pressed) {
            /*if (mc.thePlayer.ticksExisted % 4 == 1) {
                mc.thePlayer.motionY = 0.4195464;
                mc.thePlayer.setPosition(mc.thePlayer.posX + ((!(mc.thePlayer.moveForward > 0 || mc.thePlayer.moveStrafing > 0)) ? 0.005 : 0), mc.thePlayer.posY, mc.thePlayer.posZ - ((!(mc.thePlayer.moveForward > 0 || mc.thePlayer.moveStrafing > 0)) ? 0.005 : 0));
            } else if (mc.thePlayer.ticksExisted % 4 == 0) {
                mc.thePlayer.motionY = -90;
                mc.thePlayer.setPosition(mc.thePlayer.posX - ((!(mc.thePlayer.moveForward > 0 || mc.thePlayer.moveStrafing > 0)) ? 0.005 : 0.00), mc.thePlayer.posY, mc.thePlayer.posZ + ((!(mc.thePlayer.moveForward > 0 || mc.thePlayer.moveStrafing > 0)) ? 0.005 : 00.0));
            }*/
            ...
            if (e.getState() == EventMotion.State.PRE)
            {
                ...
                eventMove(e);
                if ((mc.gameSettings.keyBindJump.isPressed()) && (bool1) && (mc.thePlayer.moveForward == 0.0F) && (mc.thePlayer.moveStrafing == 0.0F)) {
                    eventMotion(e);
                }
            }
            if ((isBlockAccessible(localBlock)) && (localBlockData != null))
            {
                ...
                if (e.getState() == EventMotion.State.PRE)
                {
                    ...
                    if ((!mc.gameSettings.keyBindJump.isPressed()) && (mc.thePlayer.onGround) && (isNotCollidingBelow(0.001D)) && (mc.thePlayer.isCollidedVertically)) {
                        e.setGround(false);
                        grounded = false;
                    }else{
                        grounded = mc.thePlayer.onGround;
                ...
                else
                {
                    ...
                    updateHotbar();
                    mc.playerController.onPlayerRightClick(mc.thePlayer, mc.theWorld, mc.thePlayer.inventory.getCurrentItem(), localBlockData.pos, localBlockData.facing, blockDataToVec3(localBlockData.pos, localBlockData.facing));
    ...
    public void onPacket(EventPacket e){
        if(e.getPacketType() == EventPacket.PacketType.RECEIVE){
            ...
            int k;
            if(Lime.setmgr.getSettingByName("Spoof").getValString().equalsIgnoreCase("Spoof")) {
    ...
    public void onSend(EventPacket e){
        if(headTurned && e.getPacketType() == EventPacket.PacketType.SEND){
    ...
    {
        float f1 = Minecraft.getMinecraft().thePlayer.rotationYaw * 0.017453292F;
        float f2 = Minecraft.getMinecraft().thePlayer.rotationYaw * 0.017453292F - 4.712389F;
        float f3 = Minecraft.getMinecraft().thePlayer.rotationYaw * 0.017453292F + 4.712389F;
        float f4 = Minecraft.getMinecraft().thePlayer.rotationYaw * 0.017453292F + 0.5969026F;
        float f5 = Minecraft.getMinecraft().thePlayer.rotationYaw * 0.017453292F + -0.5969026F;
        float f6 = Minecraft.getMinecraft().thePlayer.rotationYaw * 0.017453292F - 2.3876104F;
        float f7 = Minecraft.getMinecraft().thePlayer.rotationYaw * 0.017453292F - -2.3876104F;
        if ((Minecraft.getMinecraft().gameSettings.keyBindForward.pressed) && (!isMoving()))
        {
            if ((Minecraft.getMinecraft().gameSettings.keyBindLeft.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindRight.pressed))
            {
                Minecraft.getMinecraft().thePlayer.motionX -= MathHelper.sin(f5) * paramDouble;
                Minecraft.getMinecraft().thePlayer.motionZ += MathHelper.cos(f5) * paramDouble;
            }
            else if ((Minecraft.getMinecraft().gameSettings.keyBindRight.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindLeft.pressed))
            {
                Minecraft.getMinecraft().thePlayer.motionX -= MathHelper.sin(f4) * paramDouble;
                Minecraft.getMinecraft().thePlayer.motionZ += MathHelper.cos(f4) * paramDouble;
            }
            else
            {
                Minecraft.getMinecraft().thePlayer.motionX -= MathHelper.sin(f1) * paramDouble;
                Minecraft.getMinecraft().thePlayer.motionZ += MathHelper.cos(f1) * paramDouble;
            }
        }
        else if ((Minecraft.getMinecraft().gameSettings.keyBindBack.pressed) && (!isMoving()))
        {
            if ((Minecraft.getMinecraft().gameSettings.keyBindLeft.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindRight.pressed))
            {
                Minecraft.getMinecraft().thePlayer.motionX -= MathHelper.sin(f6) * paramDouble;
                Minecraft.getMinecraft().thePlayer.motionZ += MathHelper.cos(f6) * paramDouble;
            }
            else if ((Minecraft.getMinecraft().gameSettings.keyBindRight.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindLeft.pressed))
            {
                Minecraft.getMinecraft().thePlayer.motionX -= MathHelper.sin(f7) * paramDouble;
                Minecraft.getMinecraft().thePlayer.motionZ += MathHelper.cos(f7) * paramDouble;
            }
            else
            {
                Minecraft.getMinecraft().thePlayer.motionX += MathHelper.sin(f1) * paramDouble;
        ...
        else if ((Minecraft.getMinecraft().gameSettings.keyBindLeft.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindRight.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindForward.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindBack.pressed) && (!isMoving()))
        {
            Minecraft.getMinecraft().thePlayer.motionX += MathHelper.sin(f2) * paramDouble;
            Minecraft.getMinecraft().thePlayer.motionZ -= MathHelper.cos(f2) * paramDouble;
        }
        else if ((Minecraft.getMinecraft().gameSettings.keyBindRight.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindLeft.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindForward.pressed) && (!Minecraft.getMinecraft().gameSettings.keyBindBack.pressed) && (!isMoving()))
        {
            Minecraft.getMinecraft().thePlayer.motionX += MathHelper.sin(f3) * paramDouble;
            Minecraft.getMinecraft().thePlayer.motionZ -= MathHelper.cos(f3) * paramDouble;
        }
    }
    ...
    {
        if (!Minecraft.getMinecraft().theWorld.getCollidingBoundingBoxes(Minecraft.getMinecraft().thePlayer, Minecraft.getMinecraft().thePlayer.getEntityBoundingBox().offset(0.0D, -paramDouble, 0.0D)).isEmpty()) {
            return true;
        }
        return false;
    }
    ...
    {
        double d1 = paramBlockPos.getX() + 0.5D - Minecraft.getMinecraft().thePlayer.posX + paramEnumFacing.getFrontOffsetX() / 2.0D;
        double d2 = paramBlockPos.getZ() + 0.5D - Minecraft.getMinecraft().thePlayer.posZ + paramEnumFacing.getFrontOffsetZ() / 2.0D;
        double d3 = Minecraft.getMinecraft().thePlayer.posY + Minecraft.getMinecraft().thePlayer.getEyeHeight() - (paramBlockPos.getY() + 0.5D);
