...
public class SkeletonESP extends Module {
    SlideValue lineswidth = new SlideValue("Lines Width", this, 1, 1, 10, false);
    ColorPicker colorPicker = new ColorPicker("Color", this, -1);

    public SkeletonESP(){
        super("SkeletonESP", 0, Category.RENDER);
    }
    ...
    public void on3D(Event3D e){
        ...
        mc.theWorld.playerEntities.forEach(player -> drawSkeleton(e,player, new Color(colorPicker.getValue())));
    ...
    private void drawSkeleton(Event3D event, EntityPlayer e, Color color) {
        if (!e.isInvisible()) {
            float[][] entPos = entities.get(e);
            if (entPos != null && e.isEntityAlive() && isInViewFrustrum(e) && !e.isDead && e != mc.thePlayer && !e.isPlayerSleeping()) {
                GL11.glPushMatrix();
                GL11.glLineWidth(lineswidth.getFloatValue());
                Util2D.glColor(color.getRGB());
                ...
                if (entPos[3][0] != 0.0F && (e.moveForward != 0 || e.moveStrafing != 0)) {
                    GL11.glRotatef(entPos[3][0] * 57.295776F, 1.0F, 0.0F, 0.0F);
                }

                if (entPos[3][1] != 0.0F && (e.moveForward != 0 || e.moveStrafing != 0)) {
                    GL11.glRotatef(entPos[3][1] * 57.295776F, 0.0F, 1.0F, 0.0F);
                }

                if (entPos[3][2] != 0.0F && (e.moveForward != 0 || e.moveStrafing != 0)) {
