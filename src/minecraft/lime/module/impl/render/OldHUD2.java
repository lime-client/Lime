...
public class OldHUD2 extends Module {
    public OldHUD2(){
    ...
    public void hud(){
        ...
        for(Module mod : Lime.moduleManager.getModules()){
            ...
            for(int i = 0; i < 5; i++) {
                if(mod.isToggled()) {
                    if(mod.getAnim() < mc.fontRendererObj.getStringWidth(mod.getName()) + 2) { mod.setAnim(mod.getAnim() + 1); }
                    if(mod.getAnim() > mc.fontRendererObj.getStringWidth(mod.getName()) + 3) { mod.setAnim((int) mc.fontRendererObj.getStringWidth(mod.getName())); }
                }else {
    ...
    public static Color getColor(Module m, int index){
        Color color = new Color(255, 255, 255);
        switch(Lime.setmgr.getSettingByNameAndMod("Color", Lime.moduleManager.getModuleByName("HUD")).getValString().toUpperCase()){
            case "DYNAMIC":
